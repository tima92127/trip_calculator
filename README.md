...
в список позиций не попадают, хотя выглядят так же, как обычная позиция —
текст и сумма в конце строки. Строка «ИТОГ» используется для сверки: над
списком показывается, совпадает ли сумма позиций с итогом чека, и сверка
пересчитывается при ручной правке сумм. Две одинаковые позиции в одном чеке
(например, два «Закусочных» по 120 ₽) остаются двумя позициями.
