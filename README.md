...
редактируемых категорий или разделения на поездки), дополнительно накатите
`supabase-schema-receipt-photos.sql`, `supabase-schema-categories.sql`
и/или `supabase-schema-trips.sql` — они добавляют только то, чего не
хватает, не трогая существующие данные.
...
Кнопка **«📷 Скан чека»** есть и на вкладке общих расходов, и на вкладке
**«Личное»** — можно сфотографировать чек или выбрать уже готовое фото из
галереи. Приложение распознаёт текст и показывает список найденных позиций с
суммами; описание и сумму каждой позиции можно поправить.

Со вкладки общих расходов для всего чека задаётся дата, плательщик и
категория, а у каждой позиции отдельно отмечается галочками, кто из
участников за неё в ответе (можно нескольких сразу). По кнопке «Добавить
расходы» создаётся отдельный расход на каждую отмеченную позицию, поделенный
поровну между выбранными для неё участниками.

Со вкладки «Личное» чек разбирается в личные траты того участника, который
выбран в поле «Чья трата» (по умолчанию — тот, чья вкладка сейчас открыта):
без разделения между участниками, каждая позиция целиком уходит в его личные
траты.
...
Служебные строки чека («ИТОГ», «НДС», «СУММА БЕЗ НДС», способы оплаты, сдача,
слип платёжного терминала) в список позиций не попадают, хотя выглядят так же,
как обычная позиция — текст и сумма в конце строки. Строка «ИТОГ» используется
для сверки: над списком показывается, совпадает ли сумма позиций с итогом чека,
и сверка пересчитывается при ручной правке сумм. Скидка на весь чек отдельной
строкой в сверке учитывается (но сами позиции добавляются без её учёта). Две
одинаковые позиции в одном чеке (например, два «Закусочных» по 120 ₽) остаются
двумя позициями.

Разбираются распространённые форматы строки позиции: «цена × кол-во = сумма»
(кассовые чеки), «кол-во x цена» (старые кассы: «30x100») и колонка количества
между названием и суммой (ресторанные счета: «Салат модена 2 640,00» — 2 порции
на 640 ₽). Отличить такую строку от суммы с разделителем тысяч («Ноутбук
54 990.00») по ней самой нельзя, поэтому чек разбирается в нескольких
трактовках и выбирается та, у которой сумма позиций сошлась с итогом чека.
...
Да — в шапке приложения есть переключатель поездок. Каждая поездка — это
полностью отдельный набор участников, категорий, расходов, личных трат и
взаиморасчётов, они никак не пересекаются между собой. Создать новую
поездку можно кнопкой «+ Поездка» в шапке или в разделе «Настройки».
Выбранная поездка запоминается в браузере на этом устройстве — на другом
телефоне нужно будет выбрать её ещё раз (данные при этом общие для всех,
кто открывает приложение по одной ссылке).
