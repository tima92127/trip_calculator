...
Разбираются оба распространённых формата строки позиции: «цена × кол-во =
сумма» (кассовые чеки) и колонка количества между названием и суммой
(ресторанные счета: «Салат модена 2 640,00» — 2 порции на 640 ₽). Отличить
такую строку от суммы с разделителем тысяч («Ноутбук 54 990.00») по ней самой
нельзя, поэтому чек разбирается в обеих трактовках и выбирается та, у которой
сумма позиций сошлась с итогом чека.
