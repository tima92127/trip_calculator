...
Email всех участников заранее знать не нужно — каждый привязывает свою почту
сам, при первом входе.

Настройка (один раз, в вашем проекте Supabase):

1. **SQL Editor** → выполните `supabase-migration-auth.sql` (если ставите
   проект с нуля — эти же политики уже включены в `supabase-schema.sql`).
2. **Authentication → Email Templates → Magic Link** → добавьте в текст письма
   `{{ .Token }}` — иначе в письме придёт только ссылка, без кода для ручного
   ввода.

Как это выглядит для участника:
1. При попытке что-то изменить открывается окно входа → вводит свой email →
   получает письмо с кодом → вводит код. Сессия сохраняется в браузере,
   повторно логиниться не нужно (пока не выйти самому или не очистить данные
   сайта).
2. На вкладке **Люди** рядом со своим именем нажимает **«Это я»** — так email
   привязывается к конкретному участнику. Занять чужое имя нельзя: как только
   участник привязан, кнопка исчезает.
