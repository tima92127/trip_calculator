...
`supabase-js` и `chart.js` лежат локальными файлами (`vendor/supabase.js`,
`vendor/chart.js`), а не подключаются с CDN `esm.sh` в рантайме. В некоторых
регионах Cloudflare (на котором работает esm.sh) недоступен без VPN, хотя
GitHub Pages и Supabase — доступны напрямую. Раньше это ломало приложение
целиком: если esm.sh не отвечал, весь модуль со скриптом не выполнялся вовсе
(не только графики — вообще всё, включая список расходов). Скан чека
по-прежнему грузит Tesseract.js с esm.sh динамически при первом использовании
локального распознавания — это не блокирует остальное приложение, если вдруг
тоже окажется недоступно.
